...
app.use(`/cdn/${apiVersion}`, cors, express.static(join('./server/cdn')));

if (process.env.NODE_ENV === 'production') {
    app.use('/', express.static(join('./static')));
    ...
    app.get('/', (_req, res) => res.send('⭐️ Please set to production!'));
}
...
const server = app.listen(PORT, () => {
    ...
    console.log(notice.bold(`> 🚀 HTTP Proxy [Unsecure] running on PORT: ${PORT} ⭐️ [http://localhost:${PORT}]`));
});
